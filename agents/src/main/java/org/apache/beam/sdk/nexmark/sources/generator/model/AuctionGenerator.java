...
import static org.apache.beam.sdk.nexmark.sources.generator.Generator.LOG;
...
public class AuctionGenerator {
     ...
     */
    public static long lastBase0AuctionId(long eventId, GeneratorConfig config) {
        long epoch = eventId / config.proportionDenominator;
        long offset = eventId % config.proportionDenominator;
    ...
    public static long nextBase0AuctionId(long nextEventId, Random random, GeneratorConfig config) {
        ...
        long minAuction =
                Math.max(lastBase0AuctionId(nextEventId, config) - config.getNumInFlightAuctions(), 0);
        long maxAuction = lastBase0AuctionId(nextEventId, config);
    ...
    private static long nextAuctionLengthMs(
        ...
        long numEventsForAuctions =
                ((long) config.getNumInFlightAuctions() * config.proportionDenominator)
