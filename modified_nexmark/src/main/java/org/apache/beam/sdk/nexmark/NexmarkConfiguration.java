...
/**
 * Configuration controlling how a query is run. May be supplied by command line
 * or programmatically. We only capture properties which may influence the
 * resulting pipeline performance, as captured by {@link NexmarkPerf}.
 */
...
public class NexmarkConfiguration implements Serializable {
	 ...
	 */
	@JsonProperty
	public String generateEventFilePathPrefix = null;
	...
	/**
	 * If false, the summary is only output to the console. If true the summary is
	 * output to the console and it's content is written to bigquery tables
	 * according to {@link NexmarkOptions#getResourceNameMode()}.
	 */
	@JsonProperty
	public boolean exportSummaryToBigQuery = false;

	/**
	 * Control whether pub/sub publishing is done in a stand-alone pipeline or is
	 * integrated into the overall query pipeline.
	 */
	@JsonProperty
	public NexmarkUtils.PubSubMode pubSubMode = NexmarkUtils.PubSubMode.COMBINED;

	/**
	 * Control the serialization/deserialization method from event object to pubsub
	 * messages.
	 */
	@JsonProperty
	public NexmarkUtils.PubsubMessageSerializationMethod pubsubMessageSerializationMethod = NexmarkUtils.PubsubMessageSerializationMethod.CODER;
	 ...
	 * queries that join the stream to static enrichment data.
	 */
	@JsonProperty
	public String sideInputUrl = null;

	/**
	 * Gap for the session in
	 * {@link org.apache.beam.sdk.nexmark.queries.SessionSideInputJoin}.
	 */
	@JsonProperty
	public Duration sessionGap = Duration.standardMinutes(10);

	/**
	 * Number of events to generate. If zero, generate as many as possible without
	...
	@JsonProperty
	public int firstEventRate = 10000; // DEFAULT 10000

	/** Next overall event rate (in {@link #rateUnit}). */
	@JsonProperty
	public int nextEventRate = 10000; // DEFAULT 10000
	...
	/**
	 * Time in seconds to preload the subscription with data, at the initial input
	...
	@JsonProperty
	public int streamTimeout = 240; // DEFAULT 240

	/**
	 * If true, and in streaming mode, generate events only when they are due
	 ...
	 * time in the past so that multiple runs will see exactly the same event
	 * streams and should thus have exactly the same results.
	 */
	@JsonProperty
	...
	@JsonProperty
	public int avgPersonByteSize = 200; // DEFAULT 200
	...
	/** Average idealized size of a 'pageView' event, in bytes. */
	@JsonProperty
	public int avgPageViewByteSize = 100;
	...
	/**
	 * Ratio of views to 'popular', i.e. most viewed, auctions compared to all other
	 * auctions.
	 */
	@JsonProperty
	public int popularAuctionRatio = 4;

	/** Ratio of views for active viewers compared to all other people. */
	@JsonProperty
	public int activeViewersRatio = 4;
	...
	@JsonProperty
	public long windowSizeSec = 10; // DEFAULT 10
	...
	/**
	 * Delay, in milliseconds, for each event. This will peg one core for this
	...
	/**
	 * Extra data, in bytes, to save to persistent state for each event. This will
	 * force i/o all the way to durable storage to simulate an I/O-bound
	 * computation.
	 */
	@JsonProperty
	...
	/**
	 * Skip factor for query 2. We select bids for every {@code auctionSkip}'th
	 * auction.
	 */
	@JsonProperty
	...
	/**
	 * Fanout for queries 4 (groups by category id), 5 and 7 (find a global
	 * maximum).
	 */
	@JsonProperty
	public int fanout = 5;
	...
	/**
	 * Maximum waiting time to clean personState in query3 (ie maximum waiting of
	 * the auctions related to person in state in seconds in event time).
	 */
	@JsonProperty
	 ...
	 * 1000 implies every 1000 events per generator are emitted in pseudo-random
	 * order.
	 */
	@JsonProperty
	...
	/**
	 * Replace any properties of this configuration which have been supplied by the
	 * command line.
	 */
	public void overrideFromOptions(NexmarkOptions options) {
		...
		if (options.getPopularAuctionRatio() != null) {
			popularAuctionRatio = options.getPopularAuctionRatio();
		}
		if (options.getActiveViewersRatio() != null) {
			activeViewersRatio = options.getActiveViewersRatio();
		}
	}
	...
	public NexmarkConfiguration copy() {
		...
		result.activeViewersRatio = activeViewersRatio;
		result.popularAuctionRatio = popularAuctionRatio;
		result.avgPageViewByteSize = avgPageViewByteSize;
		return result;
	}

	/**
	 * Return short description of configuration (suitable for use in logging). We
	 * only render the core fields plus those which do not have default values.
	 */
	public String toShortString() {
		...
		if (pubsubMessageSerializationMethod != DEFAULT.pubsubMessageSerializationMethod) {
			sb.append(String.format("; pubsubMessageSerializationMethod:%s", pubsubMessageSerializationMethod));
		}
		...
		if (avgPageViewByteSize != DEFAULT.avgPageViewByteSize) {
			sb.append(String.format("; avgPageViewByteSize:%d", avgPageViewByteSize));
		}
		if (activeViewersRatio != DEFAULT.activeViewersRatio) {
			sb.append(String.format("; activeViewersRatio:%d", activeViewersRatio));
		}
		if (popularAuctionRatio != DEFAULT.popularAuctionRatio) {
			sb.append(String.format("; popularAuctionRatio:%d", popularAuctionRatio));
		}
		return sb.toString();
	}
	...
	@Override
	public int hashCode() {
		return Objects.hash(debug, query, sourceType, sinkType, exportSummaryToBigQuery, pubSubMode,
				pubsubMessageSerializationMethod, numEvents, numEventGenerators, rateShape, firstEventRate,
				nextEventRate, rateUnit, ratePeriodSec, preloadSeconds, streamTimeout, isRateLimited,
				useWallclockEventTime, avgPersonByteSize, avgAuctionByteSize, avgBidByteSize, hotAuctionRatio,
				hotSellersRatio, hotBiddersRatio, windowSizeSec, windowPeriodSec, watermarkHoldbackSec,
				numInFlightAuctions, numActivePeople, coderStrategy, cpuDelayMs, diskBusyBytes, auctionSkip, fanout,
				maxAuctionsWaitingTime, occasionalDelaySec, probDelayedEvent, maxLogEvents, usePubsubPublishTime,
				outOfOrderGroupSize, avgPageViewByteSize, activeViewersRatio, popularAuctionRatio);
	}
	...
	public boolean equals(@Nullable Object obj) {
		...
		if (Double.doubleToLongBits(probDelayedEvent) != Double.doubleToLongBits(other.probDelayedEvent)) {
			return false;
		}
		...
		if(avgPageViewByteSize!=other.avgPageViewByteSize) {
			return false;
		}
		if(activeViewersRatio!=other.activeViewersRatio) {
			return false;
		}
		if(popularAuctionRatio!=other.popularAuctionRatio) {
			return false;
		}
		return true;
	}
