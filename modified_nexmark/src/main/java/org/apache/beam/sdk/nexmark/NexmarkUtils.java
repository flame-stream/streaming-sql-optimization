...
public class NexmarkUtils {
	...
	public enum SourceType {
		...
		AVRO,
		/**
		 * Read from a PubSub topic. It will be fed the same synthetic events by this
		 * pipeline.
		 */
		PUBSUB,
		/**
		 * Read events from a Kafka topic. It will be fed the same synthetic events by
		 * this pipeline.
		 */
		KAFKA
	}
	...
	static String tableSpec(NexmarkOptions options, String queryName, long now, String version) {
		return String.format("%s:%s.%s", options.getProject(), options.getBigQueryDataset(),
	...
	static String tableName(NexmarkOptions options, String queryName, long now, String version) {
		...
		case QUERY_RUNNER_AND_MODE:
			...
			String tableName = String.format("%s_%s_%s_%s", baseTableName, queryName, runnerName,
	...
	public enum RateUnit {
		PER_SECOND(1_000_000L), PER_MINUTE(60_000_000L);
	...
	public enum RateShape {
		...
		/**
		 * Return inter-event delay, in microseconds, for each generator to follow in
		 * order to achieve {@code rate} at {@code unit} using {@code numGenerators}.
		 */
		...
		/**
		 * Return array of successive inter-event delays, in microseconds, for each
		 * generator to follow in order to achieve this shape with
		 * {@code firstRate/nextRate} at {@code unit} using {@code
		 * numGenerators}.
		 */
		...
		/**
		 * Return delay between steps, in seconds, for result of
		 * {@link #interEventDelayUs}, so as to cycle through the entire sequence every
		 * {@code ratePeriodSec}.
		 */
	...
	/**
	 * Set to true to capture all info messages. The logging level flags don't
	 * currently work.
	 */
	private static final boolean LOG_INFO = false;

	/**
	 * Set to true to log directly to stdout. If run using Google Dataflow, you can
	 * watch the results in real-time with: tail -f
	 * /var/log/dataflow/streaming-harness/harness-stdout.log
	 */
	...
	/**
	 * Instants guaranteed to be strictly before and after all event timestamps, and
	 * which won't be subject to underflow/overflow.
	 */
	...
	public static void setupPipeline(CoderStrategy coderStrategy, Pipeline p) {
		...
		case HAND:
			...
			registry.registerCoderForClass(PageView.class, PageView.CODER);
			registry.registerCoderForClass(RecommendedAuction.class, RecommendedAuction.CODER);
			break;
		case AVRO:
	...
	/**
	 * Return a transform which yields a finite number of synthesized events
	...
	public static PTransform<PBegin, PCollection<Event>> batchEventsSource(NexmarkConfiguration configuration) {
		return Read
				.from(new BoundedEventSource(standardGeneratorConfig(configuration), configuration.numEventGenerators));
	}

	/**
	 * Return a transform which yields a finite number of synthesized events
	 * generated on-the-fly in real time.
	 */
	public static PTransform<PBegin, PCollection<Event>> streamEventsSource(NexmarkConfiguration configuration) {
		return Read.from(new UnboundedEventSource(NexmarkUtils.standardGeneratorConfig(configuration),
				configuration.numEventGenerators, configuration.watermarkHoldbackSec, configuration.isRateLimited));
	}
	...
	public static ParDo.SingleOutput<Event, Event> snoop(final String name) {
		return ParDo.of(new DoFn<Event, Event>() {
			...
			final Counter pageViewCounter = Metrics.counter(name, "pageViews");
			...
			public void processElement(ProcessContext c) {
				...
				if (c.element().newPerson != null) {
					...
					bidCounter.inc();
				} else if (c.element().pageView != null) {
					pageViewCounter.inc();
				} else {
	...
	/**
	 * Return a transform to keep the CPU busy for given milliseconds on every
	 * record.
	 */
	...
	private static class DiskBusyTransform<T> extends PTransform<PCollection<T>, PCollection<T>> {
		...
		public PCollection<T> expand(PCollection<T> input) {
			...
			PCollection<KV<Integer, T>> kvCollection = input.apply("diskBusy.keyElements",
					ParDo.of(new DoFn<T, KV<Integer, T>>() {
			...
			return kvCollection.apply("diskBusy.generateIO", ParDo.of(new DoFn<KV<Integer, T>, T>() {
				...
				@ProcessElement
				public void processElement(ProcessContext c, @StateId(DISK_BUSY) ValueState<byte[]> state) {
					long remain = bytes;
					long now = System.currentTimeMillis();
					while (remain > 0) {
	...
	/**
	 * Return a transform to write given number of bytes to durable store on every
	 * record.
	 */
	...
	private static class GenerateSideInputData extends PTransform<PBegin, PCollection<KV<Long, String>>> {
		...
		public PCollection<KV<Long, String>> expand(PBegin input) {
			return input.apply(GenerateSequence.from(0).to(config.sideInputRowCount))
					.apply(MapElements.via(new SimpleFunction<Long, KV<Long, String>>() {
						@Override
	 ...
	 *
	 * <p>
	 * Contains pairs of a number and its string representation to model lookups of
	 * some enrichment data by id.
	 *
	 * <p>
	 * Generated data covers the range {@code [0, sideInputRowCount)} so lookup
	 * joins on any desired id field can be modeled by looking up
	 * {@code id % sideInputRowCount}.
	 */
	public static PCollection<KV<Long, String>> prepareSideInput(Pipeline queryPipeline, NexmarkConfiguration config) {
		...
		case CSV:
			checkArgument(config.sideInputUrl != null,
					"Side input type %s requires a URL but sideInputUrl not specified", SideInputType.CSV.toString());

			checkArgument(config.sideInputNumShards > 0,
			...
			tempPipeline.apply(generateSideInputData)
					.apply(MapElements.via(new SimpleFunction<KV<Long, String>, String>(
			...
			return queryPipeline.apply(TextIO.read().from(config.sideInputUrl + "*"))
					.apply(MapElements.via(new SimpleFunction<String, KV<Long, String>>(line -> {
	...
	public static void cleanUpSideInput(NexmarkConfiguration config) throws IOException {
		...
		case DIRECT:
			break;
		case CSV:
			FileSystems.delete(FileSystems.match(config.sideInputUrl + "*").metadata().stream()
					.map(metadata -> metadata.resourceId()).collect(Collectors.toList()));
			break;
		default:
			throw new IllegalArgumentException(
					String.format("Unknown type of %s clean up requested", SideInputType.class.getSimpleName()));
		}
	}
	...
	/**
	 * Return a coder for {@code KnownSize} that are known to be exactly of type
	 * {@code T}.
	 */
	...
	public static <T extends KnownSize> PCollection<KnownSize> castToKnownSize(final String name,
			PCollection<T> elements) {
