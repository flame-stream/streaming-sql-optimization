...
/**
 * An event in the auction system, either a (new) {@link Person}, a (new)
 * {@link Auction}, or a {@link Bid}.
 */
...
public class Event implements KnownSize, Serializable {
	...
	public boolean equals(@Nullable Object o) {
		...
		Event event = (Event) o;
		return Objects.equal(newPerson, event.newPerson) && Objects.equal(newAuction, event.newAuction)
				&& Objects.equal(bid, event.bid) && Objects.equal(pageView, event.pageView);
	}

	@Override
	public int hashCode() {
		return Objects.hashCode(newPerson, newAuction, bid, pageView);
	}

	/** The type of object stored in this event. * */
	public enum Type {
		PERSON(0), AUCTION(1), BID(2), PAGE_VIEW(3);
	...
	public static final Coder<Event> CODER = new CustomCoder<Event>() {
		...
		public void encode(Event value, OutputStream outStream) throws IOException {
			...
			} else if (value.pageView != null) {
				INT_CODER.encode(Type.PAGE_VIEW.value, outStream);
				PageView.CODER.encode(value.pageView, outStream);
			} else {
		...
		public Event decode(InputStream inStream) throws IOException {
			...
			} else if (tag == Type.PAGE_VIEW.value) {
				PageView view = PageView.CODER.decode(inStream);
	...
	public @org.apache.avro.reflect.Nullable @Nullable PageView pageView;
	...
	public Event(PageView pageView) {
		newPerson = null;
		newAuction = null;
		bid = null;
	...
	/**
	 * Return a copy of event which captures {@code annotation}. (Used for
	 * debugging).
	 */
	public Event withAnnotation(String annotation) {
		if (newPerson != null) {
			...
			return new Event(pageView.withAnnotation(annotation));
		}
	}
	...
	public boolean hasAnnotation(String annotation) {
		if (newPerson != null) {
			...
			return pageView.hasAnnotation(annotation);
		}
	}
